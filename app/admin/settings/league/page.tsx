...
export default function AdminLeagueSettingsPage() {
  ...
  // ✅ Use the mutation hook for saving settings
  const saveMutation = useAppMutation(
    ...
    },
    {
      ...
      invalidateKeys: [QUERY_KEYS.LEAGUE_SETTINGS, QUERY_KEYS.COMPETITION, QUERY_KEYS.LEAGUE],
      onSuccess: (data) => {
        ...
        // ✅ Also refresh the settings to ensure UI is in sync
        fetchSettings();
      },
    }
  );
  ...
  }, [selectedSeasonId, formData, saveMutation]);

  const togglePaymentRequired = useCallback(() => {
    setFormData(prev => ({ ...prev, paymentRequired: !prev.paymentRequired }));
  }, []);

  const handleEntryFeeChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setFormData(prev => ({ ...prev, entryFee: Number(e.target.value) }));
  }, []);
      ...
      <div className="space-y-4 px-3 pb-20 sm:space-y-6 sm:px-4 lg:px-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          ...
          <button
            ...
            className="flex min-h-[44px] items-center justify-center gap-2 rounded-xl border border-white/10 bg-gray-800/60 px-4 py-2 text-white transition-colors duration-150 hover:bg-gray-700/60"
          >
        ...
        <div className="rounded-2xl border border-white/10 bg-gray-800/40 p-4 shadow-2xl backdrop-blur-xl">
          ...
          <select
            ...
            className="min-h-[44px] w-full rounded-xl border border-white/10 bg-gray-900/50 px-4 py-2.5 text-white transition-colors duration-150 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 md:w-64"
          >
            {seasons.map((season) => (
        ...
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-gray-800/40 shadow-2xl backdrop-blur-xl">
          ...
          <div className="space-y-5 p-4 sm:space-y-6 sm:p-6">
            {/* Payment Required Toggle - NO animations */}
                ...
                <div className="flex items-center gap-3">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      min="0"
                      step="50"
                      className="min-h-[44px] w-full rounded-xl border border-white/10 bg-gray-900/50 py-2.5 pl-16 pr-4 text-white placeholder:text-gray-500 transition-colors duration-150 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
                    />
                  </div>
                </div>
            ...
            <button
              ...
              className="flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 py-3 font-semibold text-white shadow-lg shadow-indigo-900/30 transition-all duration-150 hover:from-indigo-700 hover:to-purple-700 disabled:opacity-50"
            >
