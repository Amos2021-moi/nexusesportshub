...
import { useEffect, useState, type FormEvent, type ReactNode, useCallback, memo } from "react";
...
const SettingCard = memo(({
  ...
  const isMobile = useIsMobile();
  const hoverClass = isMobile ? "" : "hover:border-indigo-500/30";

  return (
    <div className={`flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-gray-800/40 p-4 shadow-xl backdrop-blur-xl transition-colors duration-150 ${hoverClass}`}>
...
export default function AdminNotificationSettingsPage() {
  ...
  const handleChange = useCallback((key: keyof AdminNotificationSettings, value: any) => {
    ...
    <>
      ...
      <div className="max-w-3xl px-3 pb-20 sm:px-4 lg:px-6">
        <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
          ...
          {/* Notification Options - NO animations */}
          ...
          {/* Digest Frequency - NO animations */}
          ...
          <div className="sticky bottom-0 -mx-4 flex gap-4 border-t border-white/10 bg-gray-900/80 px-4 pb-[env(safe-area-inset-bottom)] pt-4 backdrop-blur-xl sm:mx-0 sm:bg-transparent sm:px-0 sm:backdrop-blur-none">
            <button
              type="submit"
              disabled={saving}
              className="flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-2.5 font-semibold text-white shadow-lg shadow-indigo-900/30 transition-all duration-150 hover:from-indigo-700 hover:to-purple-700 disabled:opacity-50 sm:w-auto"
            >
              {saving ? (
                <>
