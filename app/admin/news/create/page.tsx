...
import { useState, useEffect, type ChangeEvent, type FormEvent, useCallback, memo } from "react";
...
const PreviewSection = memo(({ 
  ...
  return (
    <aside className="h-fit rounded-2xl border border-white/10 bg-gray-800/40 p-4 shadow-2xl backdrop-blur-xl sm:p-5">
      ...
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-gray-900/60">
        ...
        <div className="p-4">
          ...
          <p className="mt-2 line-clamp-5 whitespace-pre-wrap text-sm text-gray-400">
            {content || "Start writing your news content to preview it here."}
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-2xl border border-white/10 bg-gray-900/40 p-4">
        ...
        <ul className="mt-3 space-y-2 text-xs text-gray-400">
          <li className="flex items-center gap-2">
            <span className={`h-2 w-2 rounded-full ${title.trim() ? "bg-green-400" : "bg-gray-600"}`} />
            Add a clear headline
          </li>
          <li className="flex items-center gap-2">
            <span className={`h-2 w-2 rounded-full ${content.trim() ? "bg-green-400" : "bg-gray-600"}`} />
...
export default function CreateNewsPage() {
  ...
  const handlePublishedChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
        ...
        <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-r from-pink-600/20 via-rose-600/20 to-purple-600/20 p-4 shadow-2xl backdrop-blur-xl sm:p-6">
          ...
          <div className="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex min-w-0 items-center gap-3">
              <Link
                href="/admin/news"
                className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-xl border border-white/10 bg-gray-900/50 text-gray-300 transition-colors duration-150 hover:bg-white/10 hover:text-white"
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                className="min-h-[44px] w-full rounded-xl border border-white/10 bg-gray-900/60 px-4 py-3 text-white placeholder-gray-400 transition-colors duration-150 focus:border-pink-500 focus:outline-none focus:ring-2 focus:ring-pink-500/30"
                required
              />
            </div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
                <label className="group flex min-h-[120px] cursor-pointer flex-1 items-center justify-center rounded-2xl border border-dashed border-white/15 bg-gray-900/40 p-4 transition-colors duration-150 hover:border-pink-500/40 hover:bg-pink-500/5">
                  <div className="text-center">
                    <UploadCloud className="mx-auto mb-2 h-8 w-8 text-pink-300 transition-transform duration-150 group-hover:-translate-y-0.5" />
                ...
                {imagePreview && (
                  <div className="relative w-full overflow-hidden rounded-2xl border border-white/10 bg-gray-900/40 lg:w-60">
                    ...
                    <button
                      type="button"
                      onClick={removeImage}
                      className="absolute right-2 top-2 flex min-h-[32px] min-w-[32px] items-center justify-center rounded-full bg-red-500 text-white transition-colors duration-150 hover:bg-red-600"
            ...
            {/* Publish Toggle */}
            <label className="flex min-h-[44px] cursor-pointer flex-col gap-2 rounded-2xl border border-white/10 bg-gray-900/40 p-4 transition-colors duration-150 hover:bg-gray-900/60 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-3 border-t border-white/10 pt-4 sm:flex-row">
              <button
                type="submit"
                disabled={loading}
                className="flex min-h-[44px] items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-pink-600 to-rose-600 px-6 py-2.5 font-semibold text-white shadow-lg shadow-pink-900/30 transition-all duration-150 hover:from-pink-700 hover:to-rose-700 disabled:opacity-50"
              >
                {loading ? (
                  <>
              ...
              <Link
                href="/admin/news"
                className="flex min-h-[44px] items-center justify-center rounded-xl bg-gray-700 px-6 py-2.5 text-gray-300 transition-colors duration-150 hover:bg-gray-600"
              >
                Cancel
              </Link>
            </div>
